...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { lesson_id, start_from_beginning = false }: StartTavusRequest = await req.json();
    ...
    // Fetch lesson with certification information
    ...
    let customGreeting = `Hello! I'm your AI tutor for "${lesson.title}" in the ${lesson.certification_modules.title} module of the ${lesson.certification_modules.certifications.title} certification. In this lesson, we'll explore the following key concepts:\n\n${conceptsList}\n\nWe'll cover each topic in detail with examples and practical applications. Are you ready to begin with the introduction?`;
    ...
    const conversationRequest: TavusConversationRequest = {
      replica_id: replicaId,
      persona_id: personaId,
      conversation_name: `Onliversity Certification: ${lesson.title}`,
    ...
    // Update progress record to track segment index
    if (!start_from_beginning && !progressRecord) {
    ...
    return new Response(
      JSON.stringify({
        ...
        conversation_id: tavusData.conversation_id,
        lesson_title: lesson.title,
        certification_title: lesson.certification_modules.certifications.title,
        resuming: !start_from_beginning && lastCompletedSegmentIndex > 0
      }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    ...
    console.error('Error in start-certification-conversation function:', error);
    return new Response(
      JSON.stringify({ 
