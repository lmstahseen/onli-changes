...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { personal_lesson_id, num_cards }: GenerateFlashcardsRequest = await req.json();
...
- Answers should be concise but informative`;
    ...
    const { data: existingFlashcards } = await supabaseClient
      .from('personal_lesson_flashcards')
      .select('id')
      .eq('personal_lesson_id', personal_lesson_id)
      .eq('student_id', user.id)
      .single();

    let flashcardsId: string;
    ...
    console.error('Error in generate-personal-lesson-flashcards function:', error);
    return new Response(
