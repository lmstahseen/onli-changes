...
import { PersonalLearningPathService, type PersonalPathLesson, type PersonalLessonProgress, type QuizQuestion, type PersonalPathFlashcard } from '../../services/personalLearningPathService';
import { supabase } from '../../lib/supabase';

const StudentPersonalPathLesson: React.FC = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('lesson');
  const [lesson, setLesson] = useState<PersonalPathLesson & { 
  ...
  const [quiz, setQuiz] = useState<{ id: number; questions: QuizQuestion[] } | null>(null);
  ...
  const [flashcards, setFlashcards] = useState<PersonalPathFlashcard[]>([]);
  ...
  const loadLesson = async (lessonId: string) => {
    try {
      ...
      // In a real implementation, we would fetch the lesson details from the API
      // For now, we'll simulate this with a mock response
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/get-personal-path-lesson/${lessonId}`,
        {
          method: 'GET',
          headers: {
      ...
      try {
        setQuizLoading(true);
        const quizData = await PersonalLearningPathService.getPersonalPathQuiz(lessonId);
        setQuiz(quizData);
      } catch (err) {
      ...
      try {
        const flashcardsData = await PersonalLearningPathService.getPersonalPathFlashcards(lessonId);
      ...
      const savedNotes = localStorage.getItem(`personal-path-notes-${lessonId}`);
      if (savedNotes) {
        setNotes(savedNotes);
      }
  ...
  const startTavusConversation = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/start-tavus-personal-path-conversation`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
          ...
          body: JSON.stringify({ personal_path_lesson_id: lesson.id }),
        }
      );
  ...
  const handleBackToPath = () => {
    if (lesson) {
      navigate(`/student/dashboard/learning-path/personal/${lesson.module.path.id}`);
    } else {
      navigate('/student/dashboard/learning-path');
    }
  };

  const markLessonComplete = async (completed: boolean) => {
    if (!lesson || !id) return;
    
    try {
      await PersonalLearningPathService.updatePersonalLessonProgress(
        lesson.id,
        completed,
        notes
      );
  ...
  const generateQuiz = async () => {
    ...
    try {
      ...
      const generatedQuiz = await PersonalLearningPathService.generatePersonalPathQuiz(
        lesson.id,
  ...
  const generateFlashcards = async () => {
    ...
    try {
      ...
      const flashcardsData = await PersonalLearningPathService.generatePersonalPathFlashcards(
        lesson.id,
  ...
  const handleNotesChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    setNotes(newNotes);
    if (lesson) {
      localStorage.setItem(`personal-path-notes-${lesson.id}`, newNotes);
    }
  };
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6">
        ...
        >
          ...
          Back to {lesson.module.path.title}
        </button>
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-gray-600">{lesson.module.path.title}</p>
          </div>
      ...
      <div>
            ...
            {activeTab === 'notes' && (
              <div>
                ...
                <textarea
                  ...
                  className="w-full h-64 p-4 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2727E6] focus:border-transparent resize-none"
                />
        ...
        <div className="mt-6">
          <button
            onClick={() => markLessonComplete(!lesson.progress?.completed)}
          ...
          >
            {lesson.progress?.completed ? (
              <>
                <Icon name="refresh-circle-bold-duotone" size={16} />
                Mark as Incomplete
              </>
            ) : (
              <>
...
export default StudentPersonalPathLesson;